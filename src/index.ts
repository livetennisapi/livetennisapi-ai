 ...
 *
 * Individual factories exist so a caller can ship three tools instead of twelve.
 * That is not a micro-optimisation: every tool's schema and description is sent
 * to the model on every single request, so an app that only needs live scores
 * pays for the other nine in tokens, on every turn, forever.
 */
...
/** Recently completed matches with final scores and winners. */
